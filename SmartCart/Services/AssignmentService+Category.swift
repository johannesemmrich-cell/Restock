...
// Item-category classification (which aisle/shelf an item belongs to) — split out of
// AssignmentService.swift so this half can carry its own target membership (Share Extension
// AND SmartCartWidgets) without dragging the receipt-parsing-dependent store-assignment half
// (`assign`/`detectStore`, which needs `ReceiptParserService`) into the widget extension.
// `Store.pendingItems` (compiled into SmartCartWidgets) needs `categoryOrder`/`category(for:)`
// for its "no learned checkout order yet" aisle-order fallback.
struct AssignmentService {

    // MARK: - Category to store-type mapping
    // Not `private`: also read directly by `assign(itemName:to:purchaseRecords:)` in
    // AssignmentService.swift, which lives in a different file now.

    static let drugstoreKeywords: Set<String> = [
        // Körperpflege
    ...
    static let varietyStoreKeywords: Set<String> = [
        // Elektro & Beleuchtung
    ...
    static let hardwareStoreKeywords: Set<String> = [
        // Handwerkzeug
    ...
    static let nonFoodCompoundEndings: [String] = [
    ...
    // MARK: - Aisle order & display

    // Reihenfolge folgt einem typischen Rundgang durch einen Supermarkt (Eingang → Kasse),
    // nicht der alphabetischen oder sonst einer technischen Ordnung — Grundlage für den
    // Sortier-Fallback in Store.pendingItems, solange für einen Artikel noch keine gelernte
    // Abhak-Reihenfolge existiert.
    static let categoryOrder = [
        "Obst & Gemüse", "Backwaren", "Fleisch & Wurst", "Milchprodukte",
        "Getränke", "Tiefkühlkost", "Konserven", "Gewürze & Backen", "Snacks", "Lebensmittel",
